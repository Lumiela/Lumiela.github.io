...
const CaseExampleSection = forwardRef((props, ref) => {
    ...
    const imageHandler = useMemo(() => createImageHandler(quillRef, 'case_examples'), [quillRef]);
    const videoHandler = useMemo(() => createVideoHandler(quillRef), [quillRef]);
    const modules = useMemo(() => getEditorModules(imageHandler, videoHandler), [imageHandler, videoHandler]);

    // 마이그레이션 과도기 충돌 방지를 위해 이미지 우선순위를 강화한 썸네일 추출 로직
    const getThumbnail = (htmlContent) => {
        if (!htmlContent) return 'https://via.placeholder.com/600x400?text=No+Image';
        
        // 1. 본문에 이미지 태그가 명확히 존재하는지 먼저 검사
        const imgRegex = /<img[^>]+src="([^">]+)"/;
        const imgMatch = imgRegex.exec(htmlContent);
        if (imgMatch && imgMatch[1]) {
            return imgMatch[1]; // 이미지가 있으면 무조건 해당 이미지 반환
        }
        
        // 2. 이미지가 없을 때만 유튜브 비디오 ID 추출 시도
    ...
    const handleSave = async () => {
        ...
        const postData = { title: generatedTitle, content, region, crop, facility_type: facilityType, area, category: 'example1' };
        ...
        <section ref={ref} id="case-example" className="section">
            <div className="sub-section">
                <header className="subsection-header">
                    <h2 className="subsection-title">촉매형 탄산가스발생기(탄사니) 적용 사례</h2>
                    {user && viewMode === 'list' && (
